...
const source=html.match(/<script>([\s\S]*?)<\/script>/)[1].replace(/installButtonGuards\(\);initNav\(\);bind\(\);initCompactUX\(\);applyOffline\(\);[\s\S]*$/,'');
...
await test('Progressive navigation unlocks systems by highest floor',()=>{
  reset(1);assert.deepEqual(plain(run('unlockedNavs().map(n=>n[0])')),['tower','generals','settings']);run('S.bestFloor=90');assert.ok(run("featureUnlocked('path')"));assert.equal(run("featureUnlocked('events')"),false);run('S.bestFloor=200');assert.equal(run('unlockedNavs().length'),16);
});
await test('Every unlocked page switches through its isolated renderer without global errors',()=>{
  reset(200);let before=run('errorLog.length');for(let id of plain(run('unlockedNavs().map(n=>n[0])')))assert.doesNotThrow(()=>run(`renderCurrentPage('${id}')`),id);assert.equal(run('errorLog.length'),before);
});
await test('Compact help never removes functional notice nodes',()=>{
  assert.ok(html.includes(".card-body > .notice:not([id])"));assert.ok(html.includes('class="notice" id="equipped"'));assert.ok(!html.includes("querySelectorAll('.card-body > .notice').forEach"));
});
await test('Ascension is available at every floor',()=>{
  reset(1);assert.equal(run('ascensionEligibility().ready'),true);assert.equal(run('ascend()'),true);assert.equal(run('S.ascensions'),1);
});
...
await test('Same-floor ascension settles zero; added floors settle incremental decayed reward',()=>{
  reset(2000);assert.equal(run('ascend()'),true);let before=run('S.ascension.bonuses.attack');assert.equal(run('ascend()'),true);close(run('S.ascension.bonuses.attack'),before);assert.equal(run('S.ascensions'),2);run('S.bestFloor=3000');let preview=run('currentAscensionReward().attack');assert.ok(preview>0);assert.equal(run('ascend()'),true);close(run('S.ascension.bonuses.attack'),before+preview);
});
...
await test('v1.1.0 migration preserves legacy multiplier and never grants it twice',()=>{
  ...
  assert.equal(run('S.version'),'1.2.1');assert.equal(run("makeSaveCode().startsWith('SGST121-')"),true);
});
await test('All legacy save prefixes and invalid import are checked',()=>{
  for(let prefix of ['SGST110','SGST1091','SGST109','SGST108','SGST107','SGST106','SGST105','SGST104','SGST103','SGST102','SGST101','SGST20','SGST19','SGST18','SGST17','SGST16','SGST15','SGST14','SGST13','SGST12','SGST11','SGST10','SGST9','SGST8','SGST7','SGST6','SGST5','DXT1','DXT2','DXT3','DXT4']){
    assert.equal(run(`loadSaveCode('${prefix}-'+b64e(JSON.stringify({...fresh(),version:'1.1.0',ascension:undefined}))).version`),'1.2.1');
  }
  assert.throws(()=>run("loadSaveCode('SGST121-'+b64e('null'))"));assert.throws(()=>run("loadSaveCode('SGST121-'+b64e('{}'))"));
});
...
await test('Corrupt current save falls back to an intact legacy backup',()=>{
  storage.clear();storage.set('sgs_idle_v1210','{broken');storage.set('sgs_idle_v1100_backup',JSON.stringify({...plain(run('fresh()')),floor:888,bestFloor:1000}));
...
const report={version:'1.2.1',engine:'Node VM executing the shipped inline game script; presentation stubbed',tests:results,passed:results.filter(t=>t.status==='PASS').length,failed:results.filter(t=>t.status==='FAIL').length};
fs.writeFileSync(path.join(root,'docs/LOGIC_TEST_V121.json'),JSON.stringify(report,null,2)+'\n');
